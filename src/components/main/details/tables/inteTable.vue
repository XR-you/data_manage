...
<script>
import {postSaleInfo,postDevInfo,postOperaInfo,postAccountInfo,postHrInfo,postMarketInfo,postSecuInfo} from '../../../../network/postData'
export default {
  name: 'inteTable',
  data() {
    return {
            ...
            reader.onload = function(event) {
              switch (values[i]) {
                case '销售部':
                    ...
                    postSaleInfo(this.$options.methods.decode()).then(res => {
                      console.log(res)
                    })
                    break;
                case '开发部':
                    postDevInfo(event.target.result.slice(29)).then(res => {
                      console.log(res)
                    })
                    break;
                case '运营部':
                    postOperaInfo(event.target.result.slice(29)).then(res => {
                      console.log(res)
                    })
                    break;
                case '财务部':
                    postAccountInfo(event.target.result.slice(29)).then(res => {
                      console.log(res)
                    })
                    break;
                case '人力资源部':
                    postHrInfo(event.target.result.slice(29)).then(res => {
                      console.log(res)
                    })
                    break;
                case '市场营销部':
                    postMarketInfo(event.target.result.slice(29)).then(res => {
                      console.log(res)
                    })
                    break;
                case '保卫部':
                    postSecuInfo(event.target.result.slice(29)).then(res => {
                      console.log(res)
                    })
              } 
            }
